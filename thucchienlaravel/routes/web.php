...
use App\Http\Controllers\Ajax\DashboardController ;// dat nhu nay de khoi trung ten voi trang controller otren 
...
Route::post('ajax/dashboad/changeStatus',[DashboardController::class,'changeStatus'])->name('ajax.dashboad.changeStatus')->middleware('admin');
Route::post('ajax/dashboad/changeStatusAll',[DashboardController::class,'changeStatusAll'])->name('ajax.dashboad.changeStatusAll')->middleware('admin');

//Backend dasboad
//Admin user dang nhaapp 

Route::get('dashboad/index',[DashboadController::class,'index'])->name('dashboad.index')->middleware('admin');
...
//phan user quan ly trong dasboad them nguoi quan ly cha han ...
...
//phan catalogue them quan ly nhom thanh vien 
...
// phan nay la them bai viet
Route::group(['prefix'=>'post'], function () {
    Route::get('index', [PostController::class,'index'])->name('post.post.index')->middleware('admin');
    Route::get('create', [PostController::class, 'create'])->name('post.post.create')->middleware('admin');
    Route::post('store', [PostController::class, 'store'])->name('post.post.store')->middleware('admin');
    Route::get('{id}/edit', [PostController::class, 'edit'])->where(['id' => '[0-9]+'])->name('post.post.edit')->middleware('admin');
    Route::post('{id}/update', [PostController::class, 'update'])->where(['id' => '[0-9]+'])->name('post.post.update')->middleware('admin');
    Route::get('{id}/delete', [PostController::class, 'delete'])->where(['id' => '[0-9]+'])->name('post.post.delete')->middleware('admin');
    Route::delete('{id}/destroy', [PostController::class, 'destroy'])->where(['id' => '[0-9]+'])->name('post.post.destroy')->middleware('admin');
});



//phan nay lam ve chuc nang ngon ngu 
